...
This catalogue is the part that travels in the repo. The **art itself is subscription/licensed and local-only**: it lives (gitignored) under `campaigns/furrious-five/assets/maps/_full/trains/`, re-downloadable from Tom Cartos onto any machine. The descriptions below let Claude pick cars for a scene without the images being committed.
