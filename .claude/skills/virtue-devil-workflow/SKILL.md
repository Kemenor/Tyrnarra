...
Taking a Virtue Devil from "virtue line on a stub card" (a row in the canonical-fourteen table in `lore/geography/bolverk.md`, a placeholder card in the `virtueDevils` block of `non-bound-gods.html`) to a fully fleshed seat-holder with the same depth as the seven Vice Demons, and optionally to a published card, inside a single session, with the user's voice setting direction at every phase boundary.
...
| **7. HTML publish** | On explicit publish signal: replace the stub card in `non-bound-gods.html` with a full card, update `pf2e-registrar.html`, `docs/site-inventory.md`, and any cross-page mentions | One-time generator rework at first publish; see Phase 7 |
...
5. **`pf2e-registrar.html`**: the within-pantheon no-sharing rule, the Bound 13's 46 domains, the demons' 25, the cross-pantheon co-grant pattern, the pending domains (several carry non-devil candidate grantors; Swarm sits with the Vermin Queen, Undeath with Betibizi), and the two never-assigned lines (Dragon, Moon).
6. **`non-bound-gods.html`**: the `viceDemons` card schema (the publish target shape) and the `virtueDevils` stub block (what gets replaced).
...
- **The Council question.** What is this devil at the table of Fourteen: the slow voice, the procedural blade, the swing seat, the one who has spoken twice in a century? And where do they stand on the standing question of the age, the uncalled debt on Ofunda's soul?
...
2. **Favored-weapon rule.** The demons each carry one distinct PF2e Advanced-category weapon. Decide the fourteen's rule: same category, or a deliberately contrasting register (the ordered side might run to disciplined, formal, or courtly weapons). One weapon per holder, distinct across the fourteen, thematically locked to the line.
...
7. **Seat at the Council of Fourteen.** Devil-specific, with no demon equivalent: their voice in consensus debate, their alliances and oppositions among the fourteen, their stance on the détente, and their position on the uncalled Ofunda debt.
...
1. **`non-bound-gods.html`**: replace the virtue line's stub card with a full card using the same field schema as the `viceDemons` entries (name, line, location, orb emoji, accent + orbBg + orbGlow, heldSince, domain, weapon, etymology, summary, the labelled sections, reputation with `#anchor` links, secretTitle / secretLabel / secret). **One-time structural note:** the current `virtueDevils` block generates all fourteen stubs from the table via a template literal; the first publish requires reworking that generator so seated devils render full cards while unseated lines keep their stubs. Do the rework once, at the first publish, and verify the thirteen remaining stubs still render.
...
3. **`pf2e-registrar.html`**: move the devil's domains from pending to granted; record cross-pantheon co-grants in the *Granted by* column; update the summary counts.
4. **`docs/site-inventory.md`**: update the `non-bound-gods.html` entry's Virtue Devils description (stub count down, seated count up, named entry summarised).
...
6. **The Council.** How does this devil vote, and why has the Justice question (the uncalled debt) not split the Hall yet, from this seat's point of view?
...
- **Breaking the Justice restraint.** Any devil's entry that makes the uncalled Ofunda debt incoherent (or calls it casually) breaks committed canon and the city's standing tension.
...
5. **The `viceDemons` block** in `non-bound-gods.html`: the publish-target card schema, anchor-link reciprocity, and the heldSince phrasing conventions.
