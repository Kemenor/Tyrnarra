...
4. **NPC art (optional, for Forge tables).** For AI **portraits** + circle-masked or **faction-framed** NPC tokens: author per-NPC appearance + prompts (`<slug>.portraits.json`), render with `gen_portraits.py` (in `tools/imageGen/`; fal.ai, model `fal-ai/flux-2`), `upload_forge.py` to the Forge Assets Library, and apply with `foundry_macro.py assign-images` (`--token-only --no-ring` for baked frames). Faction borders bake via `bake_token.py` (magenta-field render → `prep` chroma-key → `bake`); the reusable frame library is **shared world-wide** at `tools/token-frames/` (most frames are universal — domains, regions, world factions, monster types), mapped per-quest by `<quest>.token-map.json`. Needs the **fal + Forge API keys** (gitignored in `tools/keys/`, copied from Proton Drive on a fresh clone). Full runbook (Setup, the 8-step end-to-end pipeline, and the **live-combat caution** when re-skinning placed tokens): [`tools/foundryExport/README.md`](../../../tools/foundryExport/README.md).
