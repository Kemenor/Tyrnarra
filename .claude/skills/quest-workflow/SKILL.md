...
4. **NPC art (optional, for Forge tables).** Portraits render **locally and free** via the **`npc-art` skill** (invoke it; `tools/imageGen/npc_art.py`, ComfyUI/FLUX.2 on this machine's GPU, no API key): it distills each NPC's physical + clothing block into a spec and walks the user through variations → set → upscale in chat. Then `upload_forge.py` to the Forge Assets Library and `foundry_macro.py assign-images` (`--token-only --no-ring` for baked frames). **Tokens are user-directed and faction-level** — most NPCs get none; the user names which NPC gets a token and which frame. Faction borders bake via `bake_token.py` (magenta-field render → `prep` chroma-key → `bake`); the reusable frame library is **shared world-wide** at `tools/token-frames/` (most frames are universal — domains, regions, world factions, monster types), mapped per-quest by `<quest>.token-map.json`. Needs the **Forge API key** (gitignored in `tools/keys/`). Full runbook (Setup, the end-to-end pipeline, and the **live-combat caution** when re-skinning placed tokens): [`tools/foundryExport/README.md`](../../../tools/foundryExport/README.md).
