...
description: Use this skill when building or fleshing out one of the thirteen Bound-god city-states of Talan (the god-cities) in the Tyrnarra worldbuilding project, taking it from its locked seed in docs/god-city-seeds.md to full lore and a dedicated Style B HTML page. Trigger on "build [god-city]", "flesh out [god-city]", "next god-city", "promote [god-city]", the god-city pass, or any of the thirteen names: Merkavar (Commerce/Jianna), Myrria (Darkness/Araphel), Haizava (Wind/Fisaya), Eldara (Fire/Komo), Valreka (Time/Tani, formerly Denbora), Thekkavar (Knowledge/Enki), Frae City (Freedom/Cronus), Uravel (Water/Shuun), Lurrath (Earth/Sarrum), Ljosarn (Light/Iro), Lograth (Law/Forseti), Veidrath (Hunt/Hinka), Nahaskel (Chaos/Vesuna). Also trigger when working with docs/god-city-seeds.md (the seed roster), or continuing the god-city pass across sessions. This skill builds on sub-region-workflow (reuse its phase discipline, naming-stratum rules, surface-before-writing pauses, single-page-build-directly heuristic, Style B page conventions, and lore-first / commit / publish protocol) and adds the god-city-specific method: confirm the pre-locked seed and short-circuit the seed-questions, grill the depth axes (the daily-life trio, the signature movement, the god-in-the-city manifestation pattern, the two-tier folk/GM reveal), and run the two-sentence uniqueness test against the other twelve god-cities. Compose with grill-me for the depth pass. Do NOT use for ordinary sub-regions, kingdoms, or factions (use sub-region-workflow), nor for cosmology or pantheon work.
---
...
**Phase 3 — grill the depth axes** (below) one at a time via `grill-me`, each question with a recommended answer drawn from canon and the seed.
