...
- **Region-entry prose contract per [`docs/region-prose.md`](../../../docs/region-prose.md)** (v2, 2026-08-13): eye-level narration with attributed in-world quotes carrying the color (each quoted named voice gets a voice block in lore: a one-line entry in the section's **Voices** list, or a short paragraph for a recurring voice), elevation at peaks, sections ending on a fact or a hook forward, no name-glossing in prose, the chronicler's-I only for missing things, banned constructions; the GM-written Breidey (`lore/geography/floteyn/breidey.md`) is the exemplar and the ruling. **All thirteen pages predate the contract**; like the pre-contract Floteyn files, do not remediate them unprompted, but any *new* prose a maturation pass adds follows it. A maturation pass that adds a new ⚿ secret runs the sub-region-workflow's **Phase 4.5 reveal round** (candidates + recommendation + the user's section-mass call) before drafting.
