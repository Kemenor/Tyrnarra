...
> **Open question:** the repo folder is `Tyrnara` (one R) but every HTML file calls the world `Tyrnarra` (two Rs). Confirm with the user which spelling is canonical before creating new world-level references.
...
- **New or modern things** — recently founded institutions, contemporary city-states, modern organizations — are named **plainly in English**.

  Examples: *"Free City"*, *"Order of Steam"*, *"The Wayward Compass"*, *"Clearwater Exchange"*.
...
The repo exists; Pages is not yet enabled. **To enable** (one-time, in the GitHub web UI):

1. Go to the repo's **Settings → Pages**
2. Under **Source**, choose **Deploy from a branch**
3. Branch: `main`, folder: `/ (root)`
4. Save. Pages will publish at `https://<username>.github.io/<repo-name>/`

The site will use `index.html` at the repo root as its landing page (which I've created). Sub-pages are reachable at their relative paths — e.g. `/tyrnarra-primer.html`, `/millhaven/millhaven.html`.

**No build step, no Actions workflow, no `docs/` folder needed.** Just push to `main` and GitHub serves it.

---

## Working notes

- **Don't edit existing HTML files casually.** They've been hand-tuned. If a change is needed, surface the proposed diff first.
- **Don't create README.md auto-generated by me.** The existing README.md is currently a stub — leave it alone unless asked.
- **When in doubt about canon**, check `lore/glossary.md` first, then read the relevant primer HTML, then ask.
