...
**Regional cultural registers are a third stratum, not exceptions.** The Basque/Icelandic-with-drift rule covers the deep-old layer and the English-with-drift rule covers the modern layer; alongside both, every region has its own naming register a mortal born there will follow (Fenurran Latinate, Kotokoe-kitsune, Sortalde dynastic, Haizetsua-Tengu, Valrekan Tamazight, etc.). A name in a regional register is not a rule-exception and does not need meta-flagging in lore as "pre-rule" or "predates the convention". It is simply a name from where its bearer was born.

**A name's stratum is fixed by when it was given.** The three strata are eras, and a place carries its history on the surface of its names. Deep stratum (Basque/Icelandic with drift): anything named before the Crimson Rain or in the Lost Era, and things named at the hinge of 1 MR; few things were named then, and those that were held to the old ways. Regional registers crept in through the late Lost Era. The Golden Empire (560 MR – 1325 MR) made Talanese, rendered in our voice as English with drift, the naming stratum for nearly the whole continent. The Dark Era turned the regions back to their own registers, and that holds today; only now, as the rail and the Guild connect everyone, English/Talanese is creeping back for the plain reason that everyone understands it. So a house bonded in the Lost Era, one chartered under the Empire, and one founded in the Dark Era sound like three different ages, and should. Full canon: [`lore/geography/_continent.md`](lore/geography/_continent.md), *Naming strata across the eras*.

**Word-bases may be shared; personal-name structures may not.** A regional register has two parts. Its **word-base** (the real-world language we draw from: Tamazight for Valreka, Japanese for Emarrea) names its places and things, and neighbouring regions may draw on the same base. Its **personal-name structure** (the Thousand Kingdom's house-prefix + ancestry-suffix with heir-status mobility; Valreka's given + birth-whale + chosen-whale with the *u*/*ult* particle and the *Mez-*/*-ghar* life-marks) is unique to that region and no two regions are alike. The test: hearing a character's name tells you their culture, even when the family moved two generations ago and now mixes its custom with the local one. Names in a regional register use the English alphabet and drift like any other. Record each region's structure in `lore/glossary.md` under the region's block, and its word-base in the register table in `lore/geography/_continent.md`.
...
**Region entries follow [`docs/region-prose.md`](docs/region-prose.md).** Sub-region deep files, domain-file bullets, god-city sections, and their HTML mirrors carry an additional prose contract on top of the rules here: eye-level register, end-on-the-last-fact sections, user-decided section mass, no name-glossing in prose, the ornament budget, wit only in the attributed Voices, a required live tension and one uncontested defect per region, and a banned-constructions list. Read it before drafting any region entry.
...
**The GM-Written badge** is the tier above GM-Vetted: a page whose prose the GM wrote by hand (`<!-- gm-written: YYYY-MM-DD ... -->` plus a `<div class="gm-written">` pill, same placement and same CSS file). The rule is stricter than vetting: **every prose edit to a GM-Written page is a major edit**; do not rewrite, trim, or "improve" GM-written prose unless the GM expressly instructed that exact change, and revert anything beyond the instruction. Chrome-only changes are fine and keep the badge. Only the GM grants or revokes it. The same hook flags edits to these pages. GM-Written prose also serves as exemplar material (see [`docs/region-prose.md`](docs/region-prose.md)); the lore-side mirror of such a page carries a matching note at the top of the file.
