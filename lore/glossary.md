...
- **Golivander.** Pre-rule existing name (the GM's prior worldbuilding, predates the naming convention). Travelling scholar; the most-travelled chronicler of Talan. Two bodies of work in canon: the formal-philosophical ***A Traveller's Guide to Tyrnarra*** (a multi-volume reference, most-quoted on the Gods' Law in plain language) and the periodical ***The Travelling Chronicle*** (regional dispatches he sends back to literate-class subscribers across Talan, written from wherever he currently is). Full deep-dive (race, age, education, home base, scholarly stance) flagged as TBD on `docs/open-threads.md`.
