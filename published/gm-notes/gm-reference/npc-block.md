...
Two things every NPC carries at every depth: a **Level / Non-Combat Level** (so any check against them resolves) and a **physical image-seed** (so a portrait can be generated for any face in the cast).
...
| Physical · image-seed *(1–2 sentences, for art)* | ● | ● | ● |
| Physical · full paragraph *(reading prose)* | – | ○ | ● |
...
- **The Read vs the image-seed.** The Read is the gestalt: how they come across, what you'd notice first about their manner. The image-seed is the literal look, written to feed a portrait generator (the same job `portraits.json` does today). At Full depth the image-seed is just the full Physical paragraph distilled to its one or two most paintable lines, so the pipeline stays paragraph → image-seed → portrait.
- **Physical (full) + Clothing.** Reading prose, to picture the person at the table. Required at Full, optional at Semi. They also make a better portrait prompt, so writing them is never wasted.
...
> *Physical (image-seed):* stocky and ruddy, dark hair tied back, sleeves shoved to the elbow, a permanent half-smile that doesn't reach tired eyes.
...
> *Physical (image-seed):* spare and ink-stained, close-cropped grey, wire spectacles, moves around a cluttered shop without ever once looking for anything.
