...
- **Portrait prompts are distilled, not stored.** When art is generated, Claude distills the physical + clothing text into the render spec at that moment (the `npc-art` skill, local ComfyUI), and the tuned spec lives in the NPC's `<slug>.set.json` beside the art, not in the block. There is no separate image-seed field to keep in sync with the paragraph.
