...
- **`non-bound-gods.html`** — Beyond the Thirteen. Named Non-Bound Gods (Bikiargi · Zaharsuge · Epairima · Betibizi) with tier/plane pills, aspects, worshippers, etymology, and notes. Virtue Devils (Aristotelian excess/deficiency table). Vice Demons (Seven Vices). Demon-vs-Devil distinction callout. Sun-god open-thread callout. **NOTE:** still uses the pre-flip terminology *Sin Devils* / *Virtue Demons* and the retired *Muiral the Misshapen*; awaiting full canon-write pass to land Bolverk, Tunsund, the seven named Vice Demons, the canonical fourteen virtues, and the Muiral retirement.
...
- **`pf2e-registrar.html`** — PF2e Registrar. Mechanical-reference page for crunchy Tyrnarra-to-PF2e mappings. Alphabetical master table of all 61 official PF2e cleric domains. **46 domains assigned to the Bound Thirteen** under a strict no-sharing rule (each domain to the one god whose aspects align most directly). **15 Open / canon-pending** — 13 rolled back from prior non-bound assignments (Vice Demons, Virtue Devils, Generals of Corruption, Betibizi, the Corrupted God) awaiting a fresh non-bound pass; 2 (Dragon, Moon) never formally assigned. Each Open row carries an inline note naming its prior candidate where applicable. Cross-links to grand-gods, non-bound-gods, the-binding, and magic. Extensible: scaffolded with a *more sections will land here* note for future mechanical reference (anathemas, weapons consolidation, Reflection mechanics, Magitech item mechanics). Style A, starfield omitted for the administrative feel. **NOTE:** prior-candidate annotations still use pre-flip terminology (*Sin-Devil* / *virtue-demon*); awaiting full canon-write pass.
