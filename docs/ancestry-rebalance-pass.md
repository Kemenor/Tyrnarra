...
**FENURRA REWORK — ✅ lore complete (roster locked + de-humanized; HTML pending).** **The per-tribe roster is LOCKED (user: "very very good"): Draconis = Dwarf · Vexiren = Goloma · Brakkaun = Orc · Seravain = Hobgoblin**, all four united by the Dragonblood heritage (thickest in the Draconis; the scale/ember features are the heritage on dwarven stock). Slate logic: two domain-native seats (Goloma, Orc), two from across the map (Dwarf: Talan's other volcanism; Hobgoblin: the bond-pairing IS the given-word feeling), Hunt-trio deliberately incomplete (Lizardfolk absent) so Fenurra reads apart from Villtur. Human dropped entirely. Lore written + committed: `fenurra.md` (Four-peoples-one-blood block replaces the human-stock line; chronicler why-here note, old-line-in-deep-ground; four Character paragraphs open with people + feeling, cultures untouched; Brakkaun bone-vow tied to the Orc close), `ancestries.md` (Fenurran stub roster + cross-ref line + four pointer lines in the Dwarf/Goloma/Orc/Hobgoblin entries), `ehizahar.md` (Fenurra bullet roster). **HTML phase 🌐 DONE (verified over the live local server):** `fenurra.html` (site-interactions.js added; Four-Peoples-One-Blood + chronicler paragraphs in the Four Tribes intro; People line on all four tribe cards; "Mark of the Scar" relabelled "Mark of the blood", features attributed to the heritage; the ⚿ secret-era expandable *Perfect Servants for Perfect Jobs* after the tribe grid). `ancestries.html` (Fenurran-tribes people-entry inserted between Elf and Fetchling; Fenurra pointer sentences in Dwarf/Goloma/Orc/Hobgoblin bodies; Dragonblood body gains the public origin sentence + Fenurra link). `ehizahar.html` (Peoples line roster). `dragons-reach.html` ("Reach-trained Dragonblood ancestor-seeker" → "Dragonblood carrier chasing the name's folk-conflation"). site-inventory fenurra.html description expanded. **DRAGONBLOOD ORIGIN — ✅ RESOLVED (user-decided hybrid, lore committed).** General truth at every tier: **ancient adjacency** — the lines mingled the ordinary way wherever the old Wyrmkin country ran near the surface, in the deep ages before and during the conquest; the only GM twist the general line needs is the existing inversion (the blood is older than belief-formed Zaharsuge). **The Fenurran concentration is the exception, GM-tier: Elden-made.** New `⚿ GM Secret: Perfect Servants for Perfect Jobs` in `fenurra.md`: an Elden culture on the Scar (drawn by the star-metals) crossbred conquered Wyrmkin stock into four mortal servant-lines for four perfect jobs; forge-stock dosed deepest (hence Draconis-thickest); the four complementary war-roles + the efficiency creed are the programme's unrecognized ghost; the Elden became the Corrupted God, so Fenurra's blood was bred by the thing under the world; evidence sleeps in Elden workings beneath the Scar. Secret stays REGIONAL: the heritage at large keeps clean hands. Cross-wired: ancestries.md Dragonblood entry (public origin sentence + ⚿ pointer), timeline.md Wyrmkin secret (blood-in-mortal-veins paragraph + pointer).
...
**ZAHARSUGE REWORK — ✅ lore done (2026-06-11, with the Fenurra rework).** `gods.md` sheet: the living-lineage
line inverted to the rebalance (Dragonets dominant in Brauogi as Sarrum's miners/metal-wrights, lineages
*trailing into* upper Darklands + Basogur; the god's deep cultic gravity untouched) and extended to name the
**Dragonblood** heritage as the line in mortal veins, Fenurra-strongest. Consistency fix in BOTH `gods.md` and
`timeline.md`: the Wyrmkin "held the serpentine domain... early Gods' Era" slip corrected to "into the Elden
Era's early centuries" (the conquest precedes the Gods' Era). Dragonblood origin resolved (see the Fenurra
block). Public-vs-GM split on the progenitor claim verified as already correct (folk progenitor-tradition
public; belief-formed inversion GM-tier). HTML 🌐 DONE: layer-3-gods.html Zaharsuge card
note-field inverted (Dragonets dominant in Brauogi, lineages trailing into Darklands/Basogur; Dragonblood
heritage named, Fenurra-linked) + the era slip fixed in the secret field; fenurra.html mirrored (see the
Fenurra block). **The Fenurra + Zaharsuge pair is fully closed, lore and HTML.**
...
**HTML PHASE 🌐 DONE (verified over the live local server):** `ancestries.html` Heritages section fully rebuilt in the people-entry pattern (five family h3 groups, one carry-led entry per heritage, all old ref-tables retired, Nephilim marks-callout kept; source links carry `#suzar`-style fragments — these anchors are runtime-assigned by `god-cards.js` (card.id slugified from the name, plus a deferred hash-scroll handler), so static greps for `id="suzar"` come up empty while the deep-links work fine; the registrar's fragment links were never dead). `pf2e-registrar.html` all 19 heritage Where-rows re-sourced (rarity pills untouched, mechanics pass still pending). `lost-kingdom.html` Duskwalker callout fixed (was "appear fully grown", now child-manifestation per canon; wound/door duality + Voroir Daua courtesy added). `brauogi.html` + `nahaskel.html` mention-checked, already compatible, untouched. `docs/site-inventory.md` ancestries.md + ancestries.html descriptions updated.
...
## § God-cities reconciliation — 🔵 in progress (one city at a time)

**DIRECTIVE (user, supersedes the cleanup framing):** every divine city notes **all** the ancestries of its domain, each with a little city-culture (how the feeling lands in THIS city). "Belongs to no one people" is **Frae City's distinction alone** (all of Talan equally); the other twelve carry their domain peoples. Method: written cities one by one (surface prose → yes → lore + HTML + commit); the four unbuilt seeds afterwards (print the seed notes **verbatim** when surfacing, no summarizing). Done before this directive: Thekkavar / Nahaskel / Eldara (peoples-structured at their builds).

- **Myrria ✅ (lore + HTML committed).** "Belongs to no one people" line replaced in myrkono.md + myrkono.html; *Peoples of the city* block added (myrkono.md Myrria entry + myrria.html new section): Fetchlings = the city's living memory of its purpose (Lantern Stairs shrine-keepers, never-ask innkeepers, Sanctum clergy lean Fetchling); **Centaurs RESIDENT, not visiting** (user correction: Myrria is THE city in Myrkono) = the city's stride, valley crescent + ramped lower terraces permanent clan-quarters, outriders/post/escort, first-stair token custom; Surki = the mountain's inside, underways + the true intake (fed at a Surki hearth before any registrar), the two clocks (re-named in a day, *we* years later). Accepted "for now": **queued for a grill-me pass later.**
- **Valreka ✅ (lore + HTML committed).** Nagaji = the missing third tense, **as WRIGHTS, not clergy** (user pushed back twice: draft 1 had a no-migration violation, "come down from Hareaveldi" → founding presence, aboard since the faithful took to the whales; draft 2 "read very clergy like" → reworked to working hands). Final shape: Valreka's wrights raise first halls on newly bonded whales, rig the contraption-bridges, and **build read-to-the-end Oroiri fragments into the living city** (new canon: the buried city becomes Valreka piece by piece); they claim the say over **what the city sheds** (user addition, tension), relishing the deciding in a way the keepers find faintly indecent; they close the digs (shed-skin custom: what we were stays with what was). Three-way working tension: keepers hold / wardens move / wrights transform. lioaru.md paragraph after the Azarketi; valreka.html third Peoples card ("Nagaji · the Wrights").
- **Merkavar ✅ (feel-check, lore + HTML committed).** User framing: Lautara's trio is locked, so the check is FEEL-fidelity. Verdict: Vishkanya (shaping hand → fine-artisan guilds) and Kitsune (chosen reveal → information-stalls, "what she keeps is hers" as resisting categorisation) pass as written; the Halfling sections were economy-only (blocs, freight windows) with the road-feeling unnamed. Fix: one feel-sentence at the head of the halflings-and-trains paragraph (lautara.md + merkavar.html): the city as a place the road delivers them to, the settlers settling "in the one quarter where the world keeps arriving" (the locked feeling's own proof-line, refracted).
- **Haizava ✅ (feel-check, lore + HTML committed).** Strix (ride → the city's pilots) and Kashrishi (watch → observation-discipline urban, mappers-in-the-unmappable irony) passed as written. Two cards FAILED the feel and were rewritten: **Jotunborn** had been flattened into a reader ("reading the great winds") → now *stand in it*: the crews still out when the shift-bell empties the streets, working counterweights and rails mid-shift; proof: the three-word forecast, "it will pass." **Tengu** had been flattened into knowing → now *answer it*: voice, the forecasts carried street to street, the morning-after song that tells a district its new shape from the roof-peaks. vindul.md Peoples clauses + haizava.html cards both fixed. **GM-Vetted badge STRIPPED from haizava.html** (more-than-minor prose change; the GM re-vets).
- **Frae City ✅ verified, no change** ("No dominant people; the most diverse city on Talan... all equal at the top of the climb" — the all-equally distinction, now uniquely Frae City's).
- **Lograth ✅ NO CHANGE (user rejected both proposals).** Lessons: (1) seed notes carry build-direction, never roster+feelings recaps (lookup duplication: "we're going to look the ancestry up anyway"); same for stub Peoples rows. (2) **CANON ERROR caught by user:** the proposed "the Order of Law holds a Lograth judgment after the gavel" handed an outside institution dominion inside a god-city. God-cities are sovereign; no kingdom or institution holds dominion over them, including the polity that surrounds them. The Order of Law works the domain's framework, never inside Forseti's city.
- **Seeds ✅ (minimal stale-fixes only, user-approved on full old/new verbatim surface):** Uravel untouched (roster correct); Veidrath "four Villtur tribes Orc/Kholo/Lizardfolk/Centaur" → "the Villtur peoples, Orc / Lizardfolk / Goloma"; Lurrath "Goblin/Hobgoblin/Minotaur three-way" → "Minotaur / Kholo / Dragonet three-way"; Ljosarn "Nagaji-and-Shoony" → "the Shoony / Leshy / Sprite trio combined at build" + the Egulon-pass placement decision recorded (Harro Distiratsue, on the lake bordering Argia Esfera; previously only in this delete-after-pass tracker).
- **THE GOD-CITIES RECONCILIATION IS COMPLETE: all thirteen checked against the rebalanced board** (Thekkavar/Nahaskel/Eldara at their builds; Myrria/Valreka/Merkavar/Haizava this pass; Frae City/Lograth/Uravel verified; Veidrath/Lurrath/Ljosarn seed-fixed). Myrria queued for a later grill-me pass per user. **METHOD (user, third repetition, now in memory): surfacing means the WHOLE text verbatim in the final message, old AND new when editing; never summarize, truncate, or describe.**
