...
## § Heritages pass — 🔵 grilling (divine-blood batch approved as draft; source-architecture under review)
...
**ARCHITECTURE UNDER REVIEW (user, after batch approval; decision pending):** step back on the heritage-to-source mapping. Pre-remaster, every god flavoured a heritage; the remaster shuffled and collapsed them, leaving 5 god-flavoured divine-bloods, 4 elemental-god-affinity sparks, Suli general, Ardande/Talos assigned to Feyworld/Shadowplane, Nephilim universal, Dhampir extra (Dragonblood/Hungerseed/Reflection already assigned and out of scope). Options on the table: **(a)** every god gets their own heritage; Nephilim narrows to devil/other-plane blood; **(b)** de-god entirely: heritages sourced to non-god things, Nephilim the only god-inspired line; **(c)** planar-strata architecture (recommended in chat): divine five stay god-borne; the seven sparks (Ifrit/Undine/Oread/Sylph/Ardande/Talos/Suli) re-anchor to the **Nine Primordials** (Suzar/Urzar/Lurzahar/Haizar/Zurzar/Burdinzar, Suli the blend); Nephilim stays universal; the four no-bloodline gods (Iro/Enki/Jianna/Cronus) stay godless as deliberate theology. The divine-blood prose below survives (a) and (c) unchanged.

### Approved draft — the divine-blood six (carry-register prose, destined for `ancestries.md` once the architecture settles)

**Register frame** (to follow the Versatile Heritages section intro):
