...
9. **Lautara (Commerce)** ✅ — audited + feelings committed. Trio: Halfling = *the road in the blood* / Kitsune = *the chosen reveal* / Vishkanya = *the shaping hand*. Sub-region text untouched (it IS the reference implementation; job-assignments live at place level, correctly). Halfling/Kitsune/Vishkanya lookup entries rewritten to feeling + pointer. Ratfolk/Gaps removal executed in Rika Tikur *Peoples* passage (open-threads bullet closed).
